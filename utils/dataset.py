...
def graph_partition(args):
    ...
    for category in ['train', 'val', 'test']:
        cat_data = np.load(os.path.join(f'{args.dataset_path}/{args.dataset}', category + '.npz'))
        data['x_' + category] = cat_data['x']
        data['y_' + category] = cat_data['y']

    # load static graph
    file_path = f'{args.dataset_path}/{args.dataset}/adj_mx.pkl'
    try:
    ...
    # graph partition
    try:
        part = torch.load(f'{args.dataset_path}/{args.dataset}/partition_clients_{str(num_clients)}.pt')
    except:
        if num_clients == 1 :
            part = [0] * adj_matrix.shape[0]
        else:
            n_cuts, part = metis.part_graph(G, num_clients)
        torch.save(part, f'{args.dataset_path}/{args.dataset}/partition_clients_{str(num_clients)}.pt')
...
def prepare_x_y(x, y, device, input_dim=1, output_dim=1):
    """
